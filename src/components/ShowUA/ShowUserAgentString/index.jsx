...
const copyUserAgent = () => {
  try {
    const copyText = document.getElementById("copyUserAgent")
    copyText.select()
...
const ShowUserAgentString = ({ className }) => {
  const [state, dispatch] = useReducer(reducer, defaultState)

  useEffect(() => {
    if (!state.useragent) {
      axios
        .get("https://useragent.myip.info")
        .then(({ data: payload }) => {
          dispatch({ type: "SET_USER-AGENT_VALUE", payload })
        })
        .catch(payload => dispatch({ type: "SET_USER-AGENT_FAILED", payload }))
    }
  }, [state.useragent, dispatch])
  ...
  return (
    <div className="field">
      ...
      <div className="control has-text-centered">
        <ShowUserAgentString className={className} />
      </div>
    </div>
  )
}
